...
export default function ReviewTransaction() {
  ...
  const passedTxns    = location.state?.transactions  ?? [];   // mapped (camelCase) transactions
  ...
  const [localTransactions, setLocalTransactions] = useState(passedTxns);
  ...
  const handleDelete = async (indexToDelete) => {
    ...
    try {
      if (txn?.id) {
        await api.delete(`/reports/transactions/del/${txn.id}`);
      }
  ...
  const handleSubmitAll = async () => {
    ...
    try {
      ...
      await api.post(`/reports/stocks/submit/${stockBook.report_id}`);  // ← use this
